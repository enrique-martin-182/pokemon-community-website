...
export function AudioProvider({ children }: { children: React.ReactNode }) {
  ...
  // 👇 CAMBIO: visible por defecto la primera vez (si no hay nada guardado)
  ...
  useEffect(() => {
    ...
    const onEnded = () => { if (!el.loop) next() }
    const onError = () => { setError('No se pudo cargar el audio (revisa /public/audio/...)'); setIsPlaying(false) }
  ...
  useEffect(() => {
    if (track) try { localStorage.setItem(LAST_KEY, track.id) } catch {}
  }, [track?.id])

  useEffect(() => {
    try { localStorage.setItem(VIS_KEY, visible ? '1' : '0') } catch {}
  }, [visible])
  ...
  function seekRel(delta: number) { seekAbs(currentTime + delta) }
  ...
  function next() {
    if (shuffle) { setIndex(i => randIndex(i)); return }
  ...
  function toggleVisible() { setVisible(v => !v) }

  const value: AudioCtxType = {
    tracks: TRACKS,
    index,
    track,
    isPlaying,
    ...
    playTrack, seekAbs, seekRel,
    setVolume, setLoop, setShuffle,
